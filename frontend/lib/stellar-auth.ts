...
const getEnvNetwork = () => process.env.NEXT_PUBLIC_STELLAR_NETWORK || 'TESTNET';
...
export async function getFreighterPublicKey(): Promise<string> {
    ...
    if (publicKey && typeof publicKey === 'object' && 'error' in publicKey) {
        throw new Error((publicKey as any).error as string);
    }
...
export async function signChallengeXdr(challengeXdr: string): Promise<string> {
    ...
    try {
        const signedResponse = await signTransaction(challengeXdr, {
            networkPassphrase: network === 'PUBLIC' ? 'Public Global Stellar Network ; September 2015' : 'Test SDF Network ; September 2015',
        });
        ...
        if (signedResponse && typeof signedResponse === 'object' && 'error' in signedResponse) {
            throw new Error((signedResponse as any).error as string);
        }
...
export async function verifySignature(walletAddress: string, challenge: string, signature: string): Promise<any> {
